...
dat <- filter(dat, abs(forcing) <= 20)

# run linear model with common slope
lin_common_slope <- lm(changeNL_1996_2000 ~ treatment + forcing, data = dat)
summary(lin_common_slope)

# plot relationship
plot(dat$forcing, dat$changeNL_1996_2000, 
...
```{r, message=F}
# run cubic model 
quad <- lm(changeNL_1996_2000 ~ treatment + forcing + I(treatment*forcing) + I(forcing^2) + I(treatment*(forcing^2)), data = dat)
summary(quad)
...
<div style="color:blue">
The bandwidth chosen by the algorithm is much smaller than the one chosen by us. As a consequence the samples are very small now. For a bandwidth of about 3.8, the estimated effect on light emission is now positive and significant.

_Note to reviewers: Again, we did not explicitly ask for an interpretation here, so don't penalize authors who did not provide any._
</div>
<br>
